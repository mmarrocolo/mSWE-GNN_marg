"""Wrapper: convert the 4 scaled-hydrograph warmstart simulations onto the 3-scale
template (coarsest 2000m mesh removed) and merge into one multi-sim training pkl.

Prerequisite: template_100m_3scales.pkl (already built by build_template_3scales.py).

Output:
  database/datasets/train|test/ahr_river_v03_marg_additionalsrc_velocity_100m_warmstart_{q050,q075,q125,q150}_3scales.pkl
  database/datasets/train/ahr_river_v03_marg_additionalsrc_velocity_100m_warmstart_3scales_multisim.pkl  (4 events)

The 1.0x event stays out (existing ..._warmstart_3scales dataset, used as
validation/test by config_best_sweep_multisim_multiscale_3scales.yaml with validate_on_test: True).
"""
...
TEMPLATE_PKL = os.path.join(PROJECT_ROOT, 'database', 'datasets', 'train', 'template_100m_3scales.pkl')
...
REF_1X_PKL   = os.path.join(PROJECT_ROOT, 'database', 'datasets', 'train',
                            'ahr_river_v03_marg_additionalsrc_velocity_100m_warmstart_3scales.pkl')

assert os.path.exists(TEMPLATE_PKL), \
    f'Template not found: {TEMPLATE_PKL} — run build_template_3scales.py first'
assert os.path.exists(REF_1X_PKL), \
    f'1.0x 3scales dataset not found: {REF_1X_PKL} — run run_convert_warmstart_3scales.py first'
...
# --- 1. convert each scaled simulation onto the 3-scale template ---
